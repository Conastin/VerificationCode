...
[![Version](https://img.shields.io/badge/version-2.6.0-2f6b0f)](https://github.com/Conastin/VerificationCode)
...
- **浏览器扩展（MV3）**：onnxruntime-web WASM 本地推理（零外网依赖），页面内**可视化点选配置**（右键菜单或扩展图标启动浮动配置条，悬停高亮点选验证码/输入框）；图片验证码本地识别，**纯文字验证码**（DOM 明文渲染，如 skylumo.cc）直接读取填充、无需模型；**失败样本自动收集**（识别失败/提交失败）并导出 ZIP 回注训练
...
`browser/extension/` 是 Manifest V3 扩展（**验证码自动识别填充**），支持两种验证码形态，均为**页面内可视化点选配置**（右键菜单或扩展图标启动浮动配置条）：

- **图片验证码**：验证码为 `<img>` 图片，识别完全在浏览器本地（onnxruntime-web WASM）；
- **纯文字验证码**：验证码以 DOM 明文文本渲染（如 skylumo.cc 的 4 位数字验证码），直接读取文本填充，**不加载模型**。
...
1. 打开验证码页面，**页面任意位置右键** → "配置本站验证码"（或点击扩展图标 → "页面内可视化配置"）；
2. 页面顶部出现浮动配置条，点"验证码元素"后在页面上**移动鼠标悬停高亮、点击选中**（选中 `IMG` 自动识别为图片验证码，其余为纯文字）；
3. 同样点选"输入框"；"更换验证码按钮"可选（验证码失败时自动点击重试）；
4. 点"保存配置"：选择器自动生成，配置立即生效（无需刷新），验证码自动填充。

**手动配置（popup 高级选项）**：扩展图标 → popup 中填写选择器（图片/纯文字），可选文本正则（默认提取 3~8 位字母数字），点"测试选择器"可在页面高亮验证。skylumo.cc 示例：文字选择器 `p.text-emerald-500`、输入框 `#verify-input`、更换验证码按钮 `button[class*="border-emerald-500"]`。

行为约定：

- 图片模式：置信度 <0.9 自动点击验证码刷新重试（最多 5 次，60 秒后重置）；手动点击验证码刷新也会触发重新识别（MutationObserver 监听 `src`）
- 纯文字模式：只填充不提交；若配置了"更换验证码按钮"且页面出现"验证码不正确"类提示，扩展自动点击更换验证码并重新填充（最多 5 次）
- 用户手动输入不会被覆盖

每个站点只需配置一次，配置按 `location.origin` 持久化在 `chrome.storage.local`（图片与纯文字模式共用同一份站点配置，popup 切换类型保存即可）。
